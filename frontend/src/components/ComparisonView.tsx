...
export default function ComparisonView({ properties, selectedIds, statuses, scenario, onSelect, onRemove, onDossier }: {
  ...
  if (!selected.length) return <section className="comparison-empty"><p className="eyebrow">Side-by-side evidence</p><h2>Select up to four homes to compare.</h2><p>Open a property evidence profile and add it to comparison.</p></section>;
  ...
  return <section className="comparison-view" aria-label="Property comparison"><div className="section-heading"><div><p className="eyebrow">Decision matrix</p><h2>Compare captured facts—not impressions.</h2></div><button type="button" className="button-dark dossier-button" onClick={onDossier}>Build dossier</button></div><div className="comparison-scroll"><table><thead><tr><th scope="col">Evidence field</th>{selected.map((property) => <th scope="col" key={property.id}><button type="button" onClick={() => onSelect(property)}>{property.title} ↗</button><button type="button" className="remove-compare" aria-label={`Remove ${property.title} from comparison`} onClick={() => onRemove(property.id)}>Remove</button></th>)}</tr></thead><tbody>{rows.map(([label, value]) => <tr key={label}><th scope="row">{label}</th>{selected.map((property) => <td key={property.id}>{value(property)}</td>)}</tr>)}</tbody></table></div></section>;
}
