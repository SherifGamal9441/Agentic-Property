...
export default function AgentRunSurface({ status, trace, properties, guidance, stats, brief, webAnswer, error, copied, onCancel, onRetry, onEdit, onSelect, onCompare, onCopy }: {
  ...
  if (status === "failed" || status === "cancelled") return <section className="agent-state dark-surface" aria-live="polite"><p className="eyebrow">{status === "cancelled" ? "Research paused" : "Research needs another moment"}</p><h2 ref={heading} tabIndex={-1}>{status === "cancelled" ? "Your brief is ready when you are." : "A fresh research pass is ready to begin."}</h2><p>{error || (status === "cancelled" ? "Your brief remains ready for another live search." : "Review the provider connection, then try again.")}</p><div className="result-actions"><button type="button" className="button-champagne" onClick={onRetry}>Run again</button>{brief && <button type="button" className="button-outline-light" onClick={onEdit}>Edit brief</button>}</div></section>;
  ...
  const outcomeHeadline = brief?.mode === "web_research" ? "Research complete" : outcome === "matches" ? `${suitable.length} ${suitable.length === 1 ? "home meets" : "homes meet"} your brief` : outcome === "conditional" ? `${eligible.length} promising ${eligible.length === 1 ? "home" : "homes"}—with evidence gaps` : "No homes align with every priority";
  ...
  const generatedRead = best
    ? `${best.title} leads this set with ${percent(best.fit_score)} fit and ${percent(best.evidence_coverage)} evidence coverage.${runner ? ` ${runner.title} follows as the next strongest option.` : ""}`
  ...
  return <section className="result-takeover dark-surface" aria-live="polite"><div className="result-intro"><p className="eyebrow">Research complete</p><h2 ref={heading} tabIndex={-1}>{outcomeHeadline}</h2><p>{brief?.mode === "web_research" ? "Live cited research is ready below." : `Your shortlist is shaped from the Aizen data snapshot${best?.dataset_snapshot_at ? ` dated ${best.dataset_snapshot_at}` : ""}.`}</p></div>
    {brief?.mode === "web_research" ? <div className="web-result"><p>{webAnswer}</p></div> : <>
      ...
      <div className="guidance-grid"><section><p className="eyebrow">Best match</p>{best ? <><button className="guidance-property" type="button" onClick={() => onSelect(best)}>{best.title} ↗</button><p>{best.matched_criteria.length ? `Matches ${best.matched_criteria.join(", ")}.` : "Review the property profile for its captured details."}</p></> : <p>Your next shortlist can begin with a new brief.</p>}</section><section><p className="eyebrow">Runner-up</p>{runner ? <><button className="guidance-property" type="button" onClick={() => onSelect(runner)}>{runner.title} ↗</button><p>{runner.matched_criteria.length ? `Matches ${runner.matched_criteria.join(", ")}.` : "Review its evidence profile before deciding."}</p></> : <p>A second eligible home will appear when available.</p>}</section><section><p className="eyebrow">Worth a closer look</p><p>{caveats.length ? caveats.join(", ") : "No known priority gaps in the captured fields."}</p><small>Open any home to explore its full profile.</small></section></div>
